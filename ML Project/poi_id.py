...
##
##data_dict = pd.DataFrame.from_dict(data_dict1, orient='index')
## print data_dict['salary']
## print data_dict['salary'].isnull()
...
print("Outlier:")
#print(sorted(identity, key = lambda x: x[1], reverse=True)[0:4]) #graph shows 3 outliers
...
for person in data_dict:
    ...
    for feature in financial_features:
        if data_dict[person][feature] == 'NaN':
            fin_NAN[person] += 1
#print (sorted(fin_NAN.items(), key=lambda x: x[1], reverse=True)[0])
## ('LOCKHART EUGENE E', 14)

# Find person whose email features are 'NaN'
email_NAN = {}
for person in data_dict:
    email_NAN[person] = 0
    for feature in email_features:
        if data_dict[person][feature] == 'NaN':
            email_NAN[person] += 1
#print (sorted(email_NAN.items(), key=lambda x: x[1], reverse=True))
